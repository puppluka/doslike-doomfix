...
void con_print(const char* text) {
    ...
    // Strip carriage returns entirely to prevent weird cursor resets
    ...
    while (line != NULL) {
        // Enforce the 63-character limit of the buffer to prevent horizontal bleeding
        strncpy(con_buffer[con_head], line, 63);
        con_buffer[con_head][63] = '\0'; 
        
        // Advance head safely
        con_head = (con_head + 1) % 128;
        
        // Grab next segment if present
        line = strtok(NULL, "\n");
    }
}
